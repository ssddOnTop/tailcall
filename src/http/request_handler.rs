...
use crate::http::request_handlers::api_request_handlers::api_request::api_request;
use crate::http::request_handlers::graphql_request::graphql_request;
...
pub async fn handle_request<T: DeserializeOwned + GraphQLRequestLike>(
  ...
  match *req.method() {
    ...
    hyper::Method::GET if req.uri().path().starts_with("/api") => api_request::<T>(req, state.as_ref()).await,
